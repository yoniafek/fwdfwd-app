...
// Auto-group travel steps into trips based on proximity and return flight detection
export function suggestTripGroupings(travelSteps) {
  ...
  // Get the trip's starting origin (where the journey began)
  const getTripOrigin = (steps) => {
    const firstFlight = steps.find(s => s.type === 'flight');
    return firstFlight ? extractCity(firstFlight.origin_name) : null;
  };
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    // Group if: within 7 days OR destinations overlap OR return flight
...
// Extract city name from a location string (removes airport codes, state, etc.)
function extractCity(locationName) {
  if (!locationName) return null;
  // Remove airport codes like "(SFO)" or "(EWR)"
  return locationName.replace(/\s*\([A-Z]{3}\)\s*/g, '').trim();
}

// Extract state abbreviation from address or location
function extractState(address) {
  if (!address) return null;
  // Match common US state abbreviations
  const stateMatch = address.match(/\b([A-Z]{2})\b(?:\s+\d{5})?/);
  if (stateMatch) return stateMatch[1];
  // Also try comma-separated format like "Brooklyn, NY"
  const commaMatch = address.match(/,\s*([A-Z]{2})(?:\s|$|,)/);
...
// Generate a trip name based on stay locations
function generateTripName(steps) {
  // Get all stays (hotels)
  const stays = steps.filter(s => s.type === 'hotel');
  
  // Extract unique stay cities
  const stayCities = [...new Set(
    stays.map(s => extractCity(s.origin_name)).filter(Boolean)
  )];
  
  // Case: No stays - use landing city from first flight
  if (stayCities.length === 0) {
    const firstFlight = steps.find(s => s.type === 'flight');
    if (firstFlight && firstFlight.destination_name) {
      return extractCity(firstFlight.destination_name);
    }
    return 'Trip';
  }
