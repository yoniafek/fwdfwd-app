...
export function extractCity(locationStr) {
  ...
  // Remove hotel chain names and common suffixes
  city = city.replace(/^(Aloft|Marriott|Hilton|Hyatt|Sheraton|Westin|Holiday Inn|Hampton Inn|Best Western)\s+/i, '').trim();
  return city || null;
}
...
export function generateTripName(steps) {
  ...
  if (stayCities.length === 0) {
    ...
    // Fallback: use origin of first step (e.g., car rental location)
    const firstStep = steps[0];
    if (firstStep) {
      return extractCity(firstStep.origin_name) || extractCity(firstStep.destination_name) || 'Trip';
    }
    return 'Trip';
  }
