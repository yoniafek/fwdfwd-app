...
export async function updateTravelStep(stepId, updates) {
  console.log('updateTravelStep called with:', { stepId, updates });
  
  // Build update object with ONLY core fields that definitely exist
  // This avoids errors from columns that haven't been migrated yet
  const updateData = {
    type: updates.type,
  ...
  // Always include end_datetime (can be null)
  if (updates.end_datetime !== undefined) {
    updateData.end_datetime = updates.end_datetime || null;
  }
  
  // Include destination for non-hotel types
  if (updates.destination_name !== undefined) {
    updateData.destination_name = updates.destination_name || null;
  }
  
  // Include carrier and confirmation (core fields)
  if (updates.carrier_name !== undefined) {
    updateData.carrier_name = updates.carrier_name || null;
  }
  if (updates.confirmation_number !== undefined) {
    updateData.confirmation_number = updates.confirmation_number || null;
  }
  ...
  // Try to update optional fields separately (if columns exist)
  // This is a workaround until migrations are run
  const optionalFields = {};
  if (updates.custom_title !== undefined) optionalFields.custom_title = updates.custom_title || null;
  if (updates.origin_address !== undefined) optionalFields.origin_address = updates.origin_address || null;
  if (updates.origin_lat !== undefined) optionalFields.origin_lat = updates.origin_lat || null;
  if (updates.origin_lng !== undefined) optionalFields.origin_lng = updates.origin_lng || null;
  if (updates.origin_terminal !== undefined) optionalFields.origin_terminal = updates.origin_terminal || null;
  if (updates.origin_gate !== undefined) optionalFields.origin_gate = updates.origin_gate || null;
  if (updates.destination_address !== undefined) optionalFields.destination_address = updates.destination_address || null;
  if (updates.destination_lat !== undefined) optionalFields.destination_lat = updates.destination_lat || null;
  if (updates.destination_lng !== undefined) optionalFields.destination_lng = updates.destination_lng || null;
  if (updates.destination_terminal !== undefined) optionalFields.destination_terminal = updates.destination_terminal || null;
  if (updates.destination_gate !== undefined) optionalFields.destination_gate = updates.destination_gate || null;
  ...
  if (Object.keys(optionalFields).length > 0) {
    try {
      ...
      console.warn('Could not update optional fields (migrations may be needed):', optionalError.message);
    }
  }
